...
class ParamMultiEnvCausalDistribution(MultiEnvCausalDistribution):
    ...
    def __init__(
        ...
        coeff_values, coeff_values_requires_grad = self._set_initial_coeffs(self.dag, device)
    ...
    def multi_env_log_prob(self, z: Tensor, e: Tensor, intervention_targets: Tensor) -> Tensor:
        ...
        for env in e.unique():
            ...
            for i in range(z.shape[1]):
                ...
                var = self.noise_stds[noise_env_idx][i] ** 2 * torch.ones_like(z_env[:, i])
    ...
    def _set_initial_noise_means(
        ...
        for e in range(num_envs):
            ...
            for i in range(dag.number_of_nodes()):
                ...
                val = random_val
                param = (nn.Parameter(val * torch.ones(1), requires_grad=not is_fixed)).to(device)
