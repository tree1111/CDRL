...
def load_huggingface_dataset(dataset_name, subsample_frac=1.0):
    ...
    if dataset_name == "tweet_eval":
        ...
        train = datasets.load_dataset(
            "financial_phrasebank", "sentences_75agree", revision="main", split="train"
        )
    ...
    if subsample_frac > 0:
        n = len(dset["train"])
        dset["train"] = dset["train"].select(
            np.random.choice(range(n), replace=False, size=int(n * subsample_frac))
        )
