...
def collider_bar_digit_scm(intervention_idx, labels):
    ...
    if intervention_idx == 0:
        ...
        color_digit = torch.normal(mean=0.5, std=0.2, size=(n_samples,1)).squeeze()  # 10 possible digits, evenly spaced means from 0 to 1
        ...
        digit_probabilities = torch.tensor([0.05, 0.05, 0.05, 0.05, 0.05, 0.15, 0.2, 0.25, 0.3, 0.35])
        ...
        color_digit = truncated_normal(intervention_idx / 5., 0.2, 0, 1, n_samples)
    ...
    for i in range(10):
        ...
        color_bar[mask] = truncated_normal(
            color_digit[i], 0.2, 0, 1, num_samples
        )
