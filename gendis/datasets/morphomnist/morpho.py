...
def _process_img_morph(img, threshold=0.5, scale=1):
    ...
    img = img.astype(np.float32)  # Ensure the image is of type float32
    img_min, img_max = img.min(), img.max()
    bin_img = (img >= img_min + (img_max - img_min) * threshold).astype(
        np.bool_
    )  # Ensure binary array
...
class ImageMorphology:
    ...
    def __init__(self, image, threshold: float = 0.5, scale: int = 1):
        ...
        self.scale = scale
        self.hires_image, self.binary_image, self.skeleton, self.distance_map = _process_img_morph(
            self.image, self.threshold, self.scale
        )
    ...
    def mean_thickness(self) -> float:
        ...
        thickness = 2.0 * np.mean(self.distance_map[self.skeleton]) / self.scale  # type: float
        return thickness

    @property
    def median_thickness(self) -> float:
        """Median thickness along the skeleton."""
        thickness = 2.0 * np.median(self.distance_map[self.skeleton]) / self.scale  # type: float
        return thickness
