...
def make_model(adjacency_matrix, intervention_targets_per_distr):
    ...
    model = MultiscaleFlow(q0, flows, merges, class_cond=False)
    return model
...
if __name__ == "__main__":
    ...
    for distr_idx in data_module.dataset.labels[:, -1].unique():
        idx = np.argwhere(data_module.dataset.labels[:, -1] == distr_idx)[0][0]
